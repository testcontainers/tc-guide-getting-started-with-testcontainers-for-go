...
Create a `testdata/init-user-db.sh` file to create CUSTOMERS table and insert the sample data as follows:
...
First, let us extract `PostgresContainer` creation logic into a separate file called `main_test.go` as follows:
...
Now create `customer_repository_test.go` file and write multiple tests using the same container as follows:
